...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{user-need.pdf}
...
\end{table}
\subsection{运行状态查询测试}
本小节对运行状态查询功能进行了测试，共设计了4条测试用例，分别对应4种查询功能需求，测试结果如表\ref{test-search}所示。
\begin{table}[htbp]
    \centering
    \caption{运行状态查询测试用例}
    \label{test-search}
...
在进行运维工作时，运维人员会有不同的搜索需求。本系统实现了常用的搜索方式，图\ref{search-component-path}显示了搜索符合指定路径类型关系（如vpc-slb-ecs路径类型关系）的所有拓扑路径。图\ref{search-detail-component}显示了模糊搜索符合某类型某id的组件（如id中包含字段k1aexj的vpc组件）及其周边结点。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.78\textwidth]{device-service-event.png}
